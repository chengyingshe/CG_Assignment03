...
class TemporalDataset(Dataset):
    default_cfg = {
        'dataset_path': 'data/lidarhuman26M',
        'use_aug': False,
        'use_rot': False,
        'use_straight': False,
    ...
    def update_cfg(self, **kwargs):
        ...
        for id in self.dataset_ids:
            with h5py.File(os.path.join(self.dataset_path, f'{id}.hdf5'), 'r') as f:
                # print('####f5py:', f['pose'][0])
                # assert f['pose'][0].shape == (72,), f"Dataset: {os.path.join(self.dataset_path, f'{id}.hdf5')}, the pose shape:{f['pose'][0].shape} is wrong！"
    ...
    def access_hdf5(self, index):
        ...
        for data, length in zip(self.datas, self.datas_length):
            ...
            if index > dataset_max_index:
                ...
                # print('####full_joints.shape:', full_joints.shape)
                # print('####human_points.shape:', human_points.shape)
                
                # assert pose.shape == (seqlen, 72) and full_joints.shape == (seqlen, 24, 3) and human_points.shape == (seqlen, 512, 3), 'shape is wrong！'
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.use_boundary:  # False
